...
焦点默认边框宽度7
var g_iFocusFrameWidth=11; //
var g_iFocusFrameHeight=11; //焦点默认边框高度7
...
function flyFocusStupidly(elFocusTarget,strDirection,oScrollParameter){
	...
	*/
	...
	elFocusNow.style.zIndex=elFocusNow.style.zIndex-10;
	elFocusTarget.style.zIndex=elFocusTarget.style.zIndex+10;
...
function flyFocus(elFocusTarget,strDirection,fScaleFactor,oScrollParameter){
	...
	Log.info("elFocusTargetName :::::" + elFocusTarget.id);
	...
	if (strDirection=="up" || strDirection=="down" || strDirection=="left" || strDirection=="right" ){
		...
		if( -1 != elFocusTarget.id.indexOf( "x_item" ) ){
...
function jumpFocus(elFocusTarget,fScaleFactor,oScrollParameter){ 
	...
	/*var destFocusWidth =parseInt((elFocusTarget.offsetWidth)*fScaleFactor)-1+2*g_iFocusFrameWidth+"px";
	var destFocusHeight=parseInt((elFocusTarget.offsetHeight)*fScaleFactor)-1+2*g_iFocusFrameHeight+"px";*/
...
function createAdjacency(){
	...
	for (var iStatus2 in aFocusTargets) {
		...
		//ajax('GET','http://172.0.29.70:8080/Promotion/ListByItem?',null,'aaa='+JSON.stringify(g_aAdjacency),function(){},function(){});
	  //location.href="http://172.0.29.70:8080/Promotion/ListByItem?aaa="+JSON.stringify(g_aAdjacency);
	}catch(err){
		Log.warning(err);
	}
